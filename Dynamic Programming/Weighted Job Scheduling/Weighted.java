...
class Weighted{
	public int maximum(Job[] jobs){
		...
		Arrays.sort(jobs,comparator);// Sort the jobs in finish order by weight

		T[0]=jobs[0].profit;
		for(int i=1;i<jobs.length;i++){
			T[i]=Math.max(jobs[i].profit,T[i-1]);//Initialize T[i] with max profit 
			for(int j=i-1;j>=0;j--){
				if(jobs[j].end<=jobs[i].start){//overlapping condition check
					T[i]=Math.max(T[j]+jobs[i].profit,T[i]);
					break;
